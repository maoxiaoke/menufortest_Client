...
ON_BN_CLICKED(IDC_CONNECT_BUTTON, &CmenuDlg::OnBnClickedConnectButton)
ON_BN_CLICKED(IDC_DISCONNECT_BUTTON, &CmenuDlg::OnBnClickedDisconnectButton)
//ON_WM_SIZE()
//ON_WM_SIZING()
//ON_WM_GETMINMAXINFO()
ON_WM_SIZE()
...
BOOL CmenuDlg::OnInitDialog()
{
	...
	/***改变控件的大小和位置
	CWnd *pWnd;
	pWnd = GetDlgItem(IDC_IPADDRESS); //获取控件指针，IDC_BUTTON1为控件ID号
	pWnd->SetWindowPos(NULL, 0, 0, 200, 20, SWP_NOZORDER | SWP_NOMOVE);
	*/
...
//	m_parameterset.sdDlg = &m_statusdisplydlg; //子对话框之间参数传递的说明
	...
	m_ClientSocket.ds_sdDlg = &m_statusdisplydlg;
	m_halfduplexdlg.psHDlg = &m_parameterset;
	m_halfduplexdlg.d_sdHDlg = &m_statusdisplydlg;
	m_simplexdlg.psSDlg = &m_parameterset;
	m_simplexdlg.d_sdSDlg = &m_statusdisplydlg;
...
{
	// TODO:  在此添加命令处理程序代码
	ShellExecute(this->m_hWnd, (LPCWSTR)L"open", (LPCWSTR)L" mailto:maoxiaoke@outlook.com", (LPCWSTR)L"", (LPCWSTR)L"", SW_SHOWNORMAL); //调用系统邮件
}
...
void CmenuDlg::OnBnClickedConnectButton()
{
	// TODO:  在此添加控件通知处理程序代码
	//连接服务器
	BYTE nFild[4];
...
void CmenuDlg::OnBnClickedDisconnectButton()
{
	// TODO:  在此添加控件通知处理程序代码
	//断开与服务器的连接
...
{
	m_ClientSocket.Send(P_SE, (fLen+1)*4 );  //发信息，Send发送字节长度，int型为四个字节
