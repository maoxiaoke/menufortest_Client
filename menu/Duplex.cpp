...
//				if (validframeflag == 1)
				...
				{
					pDlg->state_starthail = 0;
					pDlg->state_hailacquisition = 0;
					pDlg->state_haildirectives = 0;
					pDlg->state_hailtail = 0;
					pDlg->state_waithail = 0;
					pDlg->m_led3.SetBitmap(pDlg->m_grey);
					pDlg->m_led4.SetBitmap(pDlg->m_grey);
					pDlg->m_led5.SetBitmap(pDlg->m_grey);
					pDlg->m_led6.SetBitmap(pDlg->m_grey);
					pDlg->m_led7.SetBitmap(pDlg->m_grey);
...
//			hBmp = (HBITMAP)bitmap.GetSafeHandle();
//			pDlg->m_led1.SetBitmap(hBmp);
//			pDlg->m_led2.SetBitmap(hBmp);		//应对tab页切换灯消失的问题
...
//			break;
//		}
//		while (state_lnmdend == 1 && state_rnmdend == 1)
//		{
//			Sleep(2000);
//			state_duplexend = 1;
//			CBitmap bitmap;
...
//void CDuplex::OnTimer(UINT_PTR nIDEvent)
//{
